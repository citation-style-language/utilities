...
for style in styles:
    ...
    try:
        selfLink = styleElement.find(".//{http://purl.org/net/xbiblio/csl}link[@rel='self']").get("href")
    except:
        idLink = styleElement.find(".//{http://purl.org/net/xbiblio/csl}id").text
        info = styleElement.find(".//{http://purl.org/net/xbiblio/csl}info")
        newSelfLink = etree.SubElement(info, "link")
        newSelfLink.set("href", idLink)
        newSelfLink.set("rel", "self")
